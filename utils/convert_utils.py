...
def convert_injectable_dict_from_khoya_weight(lora_weight,separate_network_alpha=False):
    ...
    if separate_network_alpha:
        return unet_lora_dict, te_lora_dict, network_alphas
    
    for k,v in network_alphas.items():
        if k.startswith("unet."):
            unet_lora_dict[k.removeprefix(f"unet.")] = v
        elif k.startswith("text_encoder."):
            te_lora_dict[k.removeprefix("text_encoder.")] = v
    
    return unet_lora_dict, te_lora_dict
